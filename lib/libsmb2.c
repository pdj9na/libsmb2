...
static int decode_dirents(struct smb2_context *smb2, struct smb2dir *dir,
						  struct smb2_iovec *vec)
{
	...
	do
	{
		...
		SMB2_LIST_prepend(&dir->entries, ent);
					   ...
					   smb2_command_cb cb, void *cb_data)
{
	...
	//SMB2_LIST_prepend(&smb2->dirs, dir);
	dir->cb = cb;
	dir->cb_data = cb_data;
							 ...
							 smb2_command_cb cb, void *cb_data)
{
	...
	if (!smb2->user)
	{
		smb2_set_error(smb2, "No user name provided");
		return -EINVAL;
	}
	if (!smb2->password || !*smb2->password)
...
//         //这里不要释放，文件描述符要在调用smb2_close函数之后释放
//         //而不是在smb2_close函数内部释放
//         //free(fh);
// }
		...
		void *command_data, void *private_data)
{
	...
	if (status != SMB2_STATUS_SUCCESS && status != SMB2_STATUS_END_OF_FILE)
	{
		smb2_set_error(smb2, "Read/Write failed with (0x%08x) %s",
					   status, nterror_to_str(status));
		s = -nterror_to_errno(status);
	}
	else
	{
		s = rep->data_length;
		if (status == SMB2_STATUS_SUCCESS)
			rd->fh->offset = rd->offset + s;
	}

	rd->cb(smb2, s, NULL, rd->cb_data);
	free(rd);
}
			 ...
			 void *command_data, void *private_data)
{
			 ...
			 stat_data->file_info_class == SMB2_FILE_FS_FULL_SIZE_INFORMATION)
	{
		...
		statvfs->f_files = __UINT16_MAX__ + 1;
		statvfs->f_ffree = __UINT16_MAX__ + 1;
		statvfs->f_favail = __UINT16_MAX__ + 1;
		statvfs->f_namemax = __UINT8_MAX__ + 1;
	}
...
int smb2_statvfs_async(struct smb2_context *smb2, const char *path,
					   struct smb2_statvfs *statvfs, smb2_command_cb cb, void *cb_data)
{
	return smb2_getinfo_async(smb2, path,
...
static void trunc_cb_3(struct smb2_context *smb2, int status,
					   void *command_data _U_, void *private_data)
{
...
static void trunc_cb_2(struct smb2_context *smb2, int status,
					   void *command_data, void *private_data)
{
	struct trunc_cb_data *trunc_data = private_data;
...
static void trunc_cb_1(struct smb2_context *smb2, int status,
					   void *command_data _U_, void *private_data)
{
	struct trunc_cb_data *trunc_data = private_data;
...
static void rename_cb_3(struct smb2_context *smb2, int status,
						void *command_data _U_, void *private_data)
{
...
static void rename_cb_2(struct smb2_context *smb2, int status,
						void *command_data _U_, void *private_data)
{
	struct rename_cb_data *rename_data = private_data;
...
static void rename_cb_1(struct smb2_context *smb2, int status,
						void *command_data _U_, void *private_data)
{
	struct rename_cb_data *rename_data = private_data;
...
static void ftrunc_cb_1(struct smb2_context *smb2, int status,
						void *command_data _U_, void *private_data)
{
...
static void readlink_cb_3(struct smb2_context *smb2, int status,
						  void *command_data _U_, void *private_data)
{
...
static void readlink_cb_2(struct smb2_context *smb2, int status,
						  void *command_data, void *private_data)
{
	struct readlink_cb_data *cb_data = private_data;
...
static void readlink_cb_1(struct smb2_context *smb2, int status,
						  void *command_data _U_, void *private_data)
{
	struct readlink_cb_data *cb_data = private_data;
...
static void disconnect_cb_2(struct smb2_context *smb2, int status,
							void *command_data _U_, void *private_data)
{
...
static void disconnect_cb_1(struct smb2_context *smb2, int status,
							void *command_data _U_, void *private_data)
{
	struct disconnect_data *dc_data = private_data;
	struct smb2_pdu *pdu;
...
struct smb2fh *smb2_dupfh(struct smb2fh *_fh)
{
	struct smb2fh *fh;
	...
	//memcpy(fh->file_id, _fh->file_id, SMB2_FD_SIZE);
	memcpy(fh, _fh, sizeof(struct smb2fh));
	//SMB2_LIST_prepend(&smb2->fhs, fh);
