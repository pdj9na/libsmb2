...
int smb2_which_events(struct smb2_context *smb2)
{
	...
	if (pdu)
		if (smb2_get_credit_charge(smb2, pdu) <= smb2->credits)
			events |= POLLOUT;
	...
	while ((pdu = smb2->outqueue))
	{
		...
		if (pdu->out.num_done == SMB2_SPL_SIZE + spl)
		{
			...
			while (pdu)
			{
				...
				SMB2_LIST_append(&smb2->waitqueue, pdu);
				pdu = tmp_pdu;
			}
		}
	}
...
read_more_data:
		...
		(smb2->hdr.command != SMB2_SESSION_SETUP))
	{
		...
		{
			smb2_set_error(smb2, "Wrong signature in received PDU");
			return -1;
		}
	}
...
int smb2_service_fd(struct smb2_context *smb2, int fd, int revents)
{
	...
	if (revents & POLLERR)
	{
		...
		else if (getsockopt(fd, SOL_SOCKET, SO_ERROR, (char *)&err, &err_size) || err)
		{
			if (err == 0)
				err = errno;
	...
	if (smb2->fd == -1 && revents & POLLOUT)
	{
		...
		if (getsockopt(fd, SOL_SOCKET, SO_ERROR, (char *)&err, &err_size) || err)
		{
			if (err == 0)
				err = errno;
			if (smb2->next_addrinfo)
	...
	if (revents & POLLOUT && smb2->outqueue)
	{
		if (smb2_write_to_socket(smb2))
		{
			ret = -1;
			goto out;
		}
