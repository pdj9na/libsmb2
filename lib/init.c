...
static int smb2_parse_args(struct smb2_context *smb2, const char *args)
{
	while (args && *args)
	{
		...
		else if (!strcmp(args, "credentials"))
		{
			if (smb2->credentials)
				free(discard_const(smb2->credentials));
			smb2->credentials = strdup(value);
		}
		else if (!strcmp(args, "password"))
			smb2_set_password(smb2, value);
		/* Android need */
		else if (!strcmp(args, "username"))
			smb2_set_user(smb2, value);
		else
		{
...
static void smbcontext_destroy_threadspecific(smbcontext_threadspecific_t *smbts)
{
	if (!smbts)
		return;
	// while (smb2->outqueue)
	...
	// 	pdu->cb(smbts->_parent, SMB2_STATUS_CANCELLED, NULL, pdu->cb_data);
	// 	smb2_free_pdu(smbts->_parent, pdu);
	// }
...
int smbcontext_init_threadspecific(struct smb2_context *_ct)
{
	...
	// if (!_ct->threadspecific_key)
	// {
	// 	_ct->threadspecific_key = (pthread_key_t)(__uint64_t)_ct;
	// 	res = pthread_key_create(&_ct->threadspecific_key,
	// 							 (void *)smbcontext_destroy_threadspecific);
	// }
	return res;
}

smbcontext_threadspecific_t *smbcontext_get_threadspecific(struct smb2_context *_ct)
{
	smbcontext_threadspecific_t *res = NULL;
	// res = pthread_getspecific(_ct->threadspecific_key);
	// if (!res)
	// {
	// 	smbcontext_threadspecific_t *_t = calloc(1, sizeof(smbcontext_threadspecific_t));
	// 	if (!_t)
	// 	{
	// 		errno = ENOMEM;
	// 		goto end;
	...
	// 	if (!pthread_setspecific(_ct->threadspecific_key, _t))
	// 	{
	// 		_t->_parent = _ct;
	// 		//_t->fd = -1;
...
struct smb2_context *smb2_init_context(void)
{
	...
	if (smb2 == NULL)
		return NULL;
	// if (smbcontext_init_threadspecific(smb2))
	// {
	// 	free(smb2);
	// 	return NULL;
	// }
...
void smb2_set_password_from_file(struct smb2_context *smb2, char *file)
{
	...
	if (file)
		name = strdup(file);
	else if (smb2->credentials)
		name = strdup(smb2->credentials);
	else
	{
...
// void smb2_set_userorpass(struct smb2_context *smb2, const char *user, const char *pass)
// {
// 	if (user)
// 	{
// 		if (smb2->user)
